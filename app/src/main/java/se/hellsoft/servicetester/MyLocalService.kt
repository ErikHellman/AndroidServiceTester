...
class MyLocalService : Service() {
    ...
    override fun onCreate() {
        super.onCreate()
        log("MyLocalService.onCreate called")
    }

    override fun onBind(intent: Intent?): IBinder? {
        log("MyLocalService.onBind called")
        return binder
    }

    override fun onUnbind(intent: Intent?): Boolean {
        log("MyLocalService.onUnbind called")
        return false
    }

    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        log("MyLocalService.onStartCommand called with startId $startId")
        startIds += startId
        val startForeground = intent?.getBooleanExtra(MainActivity.START_FOREGROUND, true) ?: true
        if (startForeground) {
            log("MyLocalService starting in foreground")
    ...
    override fun onDestroy() {
        super.onDestroy()
        log("MyLocalService.onDestroy called")
    }
    ...
    companion object {
        ...
        const val CHANNEL_ID = "local-service"
        fun defaultNotification(context: Context): Notification {
            ...
            return NotificationCompat.Builder(context, CHANNEL_ID)
                ...
                .setContentText("The Local Service is running!")
